...
local getcustomassets = {
	['vape/assets/wurst/triangle.png'] = 'rbxasset://wurst/triangle.png'
}
...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/WhichChapter/okkkkkkVape/'..readfile('vape/profiles/commit.txt')..'/'..select(1, path:gsub('vape/', '')), true) end)
...
function mainapi:CreateCategory(categorysettings)
	...
	function categoryapi:CreateModule(modulesettings)
		...
		triangle.Image = getcustomasset('vape/assets/wurst/triangle.png')
